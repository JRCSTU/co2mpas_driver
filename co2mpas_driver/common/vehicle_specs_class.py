    ...
    def __init__(self, my_car, **kwargs):
        ...
        if "electric" not in kwargs or not kwargs["electric"]:
            ...
            self.engine_max_speed_at_max_power = float(
                my_car["Fuel Engine-Max power RPM"]
            )  # rpm
            gr_str = my_car[
                "Transmission  / Gear ratio-Gear Box Ratios"
            ]  # [3.33, 1.95, 1.29, 0.98, 0.76]
            if gr_str[0] == "[":
                gr_str = gr_str[1:-1]
            gr_str = gr_str.split("-")
            gr = []
            ...
            self.veh_max_speed = round(
                int(float(my_car["Performance-Top speed"]) / 3.6), 2
            )  # m/s
            ...
            if "lco" in kwargs:
                if kwargs["lco"]:
                    ...
                    self.r_dynamic = (
                        float(my_car["Chassis-Rolling Radius Dynamic"]) / 1000
                    )
            ...
            if kwargs["electric"]:
                ...
                self.engine_max_power = float(
                    my_car["Electric Engine-Total max power"]
                )  # kW
                self.motor_max_torque = float(
                    my_car["Electric Engine-Max torque"]
                )  # Nm
                self.gr = 1
                self.tire_radius = (
                    float(my_car["Chassis-Rolling Radius Static"]) / 1000
                )  # meters
                self.driveline_slippage = 0
                if (
                    my_car["General Specifications-Transmission"]
...
class hardcoded_params(object):
    ...
    def __init__(self):
        ...
        self.params_gearbox_losses = {
            "Automatic": {
                "gbp00": {"m": -0.0043233434399999994, "q": -0.29823614099999995},
                "gbp10": {"m": -2.4525999999999996e-06, "q": -0.0001547871},
                "gbp01": {"q": 0.9793688500000001},
            },
            "Manual": {
                "gbp00": {"m": -0.0043233434399999994, "q": -0.29823614099999995},
                "gbp10": {"m": -2.4525999999999996e-06, "q": -5.15957e-05},
