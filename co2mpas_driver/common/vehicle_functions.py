...
def estimate_f_coefficients(my_car, passengers=0):
    ...
    f2 = (
        0.5
        * 1.2
        * (0.84 * my_car.car_width * my_car.car_height * theor_aero_coeff)
        / pow(3.6, 2)
    )
    f1 = -71.735 * f2 + 2.7609
...
def get_speeds_n_accelerations_per_gear(my_car, full_load_speeds, full_load_torques):
    ...
    for j in range(len(my_car.gr)):
        ...
        temp_acc = (
            full_load_torques[mask]
            * (my_car.final_drive * my_car.gr[j])
            * my_car.driveline_efficiency
    ...
    for j in range(len(my_car.gr)):
        ...
        cs_acc_per_gear.append(
            CubicSpline(
                ...
                [acc_per_gear[j][0]] * len(prefix_list)
                + list(acc_per_gear[j])
...
def get_resistances(my_car, sp_bins):
    ...
    car_res_curve, car_res_curve_force = veh_resistances(
        f0, f1, f2, list(sp_bins), my_car.veh_mass
    )
    Alimit = Armax(my_car)
    return car_res_curve, car_res_curve_force, Alimit
...
def get_spline_out_of_coefs(coefs_per_gear, starting_speed):
    ...
    """
    x_new = np.insert(
        np.arange(starting_speed, 70, 0.1), [0, 0], [0, starting_speed / 2]
    )
...
def ev_curve(my_car):
    ...
    veh_max_acc = (
        my_car.motor_max_torque
        * (my_car.final_drive * my_car.gr)
        * my_car.driveline_efficiency
        / (my_car.tire_radius * my_car.veh_mass)
    )  # m/s2
