...
def light_co2mpas_instant(
    ...
    final_drive_torque_in = func.calculate_final_drive_torques_in(
        veh_wheel_torque, final_drive, final_drive_torque_losses
    )

    gear_box_speeds_in = func.calculate_gear_box_speeds_in_v1(
        gear, final_drive_speed, gear_box_ratios, 0
    )

    gearbox_params = func.create_gearbox_params(veh_params, engine_max_torque)
    ...
    params = func.parameters(max_power, fuel_eng_capacity, fuel_type, fuel_turbo)
    ...
    if br_eff_pres > 20:
        ...
        print(
            "BMEP> %.2f bar, EngineCM: %.2f, Gear: %d : Check out the MFC output. The engine will blow up!!!!"
    ...
    fc = func.calc_fuel_consumption(
        VMEP, fuel_eng_capacity, lower_heating_value, gear_box_speeds_in, sim_step
    )
